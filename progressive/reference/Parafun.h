...
public:
	...
	~Parafun();
	bool writeobj(ProgressiveMesh& out_mesh,const string out_filename);
	bool write_reference_obj(const string& out_filename);
	...
	void set_iteration_callback(const std::function<void(Parafun&, const char*, int)>& callback) { iteration_callback = callback; }
	void set_stop_callback(const std::function<bool()>& callback) { stop_callback = callback; }


	ProgressiveMesh mesh;
	double Intp_T_Min;
	...
	std::function<void(Parafun&, const char*, int)> iteration_callback;
