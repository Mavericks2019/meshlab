...
Parafun::Parafun(string filename)
{
	if (!OpenMesh::IO::read_mesh(mesh, filename))
		throw std::runtime_error("Cannot read input mesh: " + filename);
	file_str = filename;
	modelname = file_str.substr(file_str.find_last_of("/\\") + 1);
	const size_t extension = modelname.find_last_of('.');
	if (extension != string::npos)
		modelname.erase(extension);
	if (mesh.n_faces() == 0 || mesh.n_vertices() == 0)
		throw std::runtime_error("Input mesh is empty: " + filename);
...
bool Parafun::writeobj(ProgressiveMesh& out_mesh, const string out_filename)
{
...
bool Parafun::write_reference_obj(const string& out_filename)
{
	ProgressiveMesh reference_mesh;
	const double* position = position_of_mesh.data();

	for (int i = 0; i < F_N; ++i)
	{
		const double p00 = update_p00[i];
		const double p01 = update_p01[i];
		const double p10 = update_p10[i];
		const double p11 = update_p11[i];
		const double det = p00*p11 - p01*p10;
		if (abs(det) <= numeric_limits<double>::epsilon())
			throw std::runtime_error("Degenerate interpolated reference triangle");

		// update_p stores the inverse edge matrix of each independent reference triangle.
		double rx[3] = { 0.0, p11 / det, -p01 / det };
		double ry[3] = { 0.0, -p10 / det, p00 / det };
		double qx[3] = { position[F0[i]], position[F1[i]], position[F2[i]] };
		double qy[3] = { position[F0[i] + V_N], position[F1[i] + V_N], position[F2[i] + V_N] };
		...
		for (int j = 0; j < 3; ++j)
		{
			const double x = rx[j] - rcx;
			const double y = ry[j] - rcy;
			dot += x*(qx[j] - qcx) + y*(qy[j] - qcy);
			cross += x*(qy[j] - qcy) - y*(qx[j] - qcx);
		}

		const double rotation_norm = sqrt(dot*dot + cross*cross);
		const double cos_angle = rotation_norm > 0.0 ? dot / rotation_norm : 1.0;
		const double sin_angle = rotation_norm > 0.0 ? cross / rotation_norm : 0.0;
		vector<ProgressiveMesh::VertexHandle> face_vertices;
		face_vertices.reserve(3);
		for (int j = 0; j < 3; ++j)
		{
			const double x = rx[j] - rcx;
			const double y = ry[j] - rcy;
			const double aligned_x = qcx + cos_angle*x - sin_angle*y;
			const double aligned_y = qcy + sin_angle*x + cos_angle*y;
			face_vertices.push_back(reference_mesh.add_vertex(OpenMesh::Vec3d(
				originmesh_area_sqrt*aligned_x,
				originmesh_area_sqrt*aligned_y,
				0.0)));
		}
		reference_mesh.add_face(face_vertices);
	}

	return OpenMesh::IO::write_mesh(reference_mesh, out_filename);
}
...
void Parafun::BPE()
{
	...
	if (iteration_callback)
		iteration_callback(*this, "Tutte", 0);
	...
	{
		if (stop_callback && stop_callback()) break;
		iter_num_cur++;
		energy_pre = energy_cur;
		Update_source_same_t();
		if (iter_num_cur == 1)
		{
			ProgressiveMesh current_parameterization = mesh;
			if (!writeobj(current_parameterization, modelname + "_mp.obj"))
				throw std::runtime_error("Cannot write first current parameterization");
			if (!write_reference_obj(modelname + "_mr.obj"))
				throw std::runtime_error("Cannot write first interpolated reference");
		}
		if (changetocm_flag < 0.99&&conv_percent>0.1&&Intp_T_Min<0.999)
		{
			SLIM();
			Update_source_same_t();
			if (iteration_callback) iteration_callback(*this, "Progressive SLIM", iter_num_cur);
			energy_area_process.push_back(energy_area);
			slim_iter_num++;
			sum_iter_num++;
	...
	while (iter_num_cur < MAX_ITER_NUM)
	{
		...
		if (conv_percent > 0.01&&Intp_T_Min<0.999)
		{
			CM();
			Update_source_same_t();
			if (iteration_callback) iteration_callback(*this, "Progressive CM", iter_num_cur);
			energy_area_process.push_back(energy_area);
			cm_iter_num++;
			sum_iter_num++;
		...
		else
		{
			...
			while (iter_num_cur < MAX_ITER_NUM)
			{
				...
				CM();
				if (iteration_callback) iteration_callback(*this, "Final CM", iter_num_cur);
				energy_area_process.push_back(energy_area);
				sum_iter_num++;
...
void Parafun::init()
{
	...
	for (int i = 0; i < F_N; ++i)
	{
		...
		int dd = 0;
		for (ProgressiveMesh::FaceVertexIter it2 = mesh.fv_begin(face); it2 != mesh.fv_end(face); ++it2)
		{
			auto vertex_ = *it2;
			switch (dd)
			{
...
void Parafun::run_bpe()
{
	init();
	BPE();
	writeobj(mesh, modelname + "_comp_para_result.obj");
}
