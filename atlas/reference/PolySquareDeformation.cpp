...
static OpenMesh::Vec3uc tag_colors[4] =
{
	{ 255,  41,  30 },
	{ 229, 198,  56 },
	{ 6, 216,  20 },
...
bool PolySquareDeformation::calc(double kernel, double goal, double exp)
{
	QElapsedTimer timer;
	timer.start();
	if (!begin_calc(kernel, goal, exp)) return false;

	std::string title;
	bool entered_grid_scale = false;
	while (!is_complete()) step(title, entered_grid_scale);
...
bool PolySquareDeformation::begin_calc(double kernel, double goal, double exp)
{
	kernel_width = kernel;
//	goal_factor = goal;
	goal_length = goal;
	...
	set_uv();
	for (auto& chart : atlas) chart.begin_deformation();
	chart_alignment_complete.assign(atlas.size(), false);
	align_iteration = 0;
	step_state = StepState::Rotation;
	return true;
}

bool PolySquareDeformation::step(std::string& title, bool& entered_grid_scale)
{
	entered_grid_scale = false;
	if (step_state == StepState::Idle || step_state == StepState::Complete) return false;

	switch (step_state)
	{
	case StepState::Rotation:
		for (auto& chart : atlas) chart.rotation_step();
		title = "Rotation";
		step_state = StepState::Align;
		break;
	case StepState::Align:
		for (int i = 0; i < static_cast<int>(atlas.size()); ++i)
			if (!chart_alignment_complete[i]) atlas[i].align_step(align_iteration);
		title = "Align Deformation " + std::to_string(align_iteration + 1);
		step_state = StepState::Inner;
		break;
	case StepState::Inner:
		for (int i = 0; i < static_cast<int>(atlas.size()); ++i)
			if (!chart_alignment_complete[i]) chart_alignment_complete[i] = atlas[i].inner_step();
		title = "Inner Deformation " + std::to_string(align_iteration + 1);
		++align_iteration;
		step_state = (align_iteration >= 15 ||
			std::all_of(chart_alignment_complete.begin(), chart_alignment_complete.end(), [](bool done) { return done; }))
			? StepState::Flattening : StepState::Align;
		break;
	case StepState::Flattening:
		for (auto& chart : atlas) chart.flattening_step();
		title = "Flattening";
		entered_grid_scale = true;
		step_state = StepState::Untangle;
		break;
	case StepState::Untangle:
		for (auto& chart : atlas) chart.untangle_step();
		title = "Untangle";
		step_state = StepState::Final;
		break;
	case StepState::Final:
		for (auto& chart : atlas) chart.final_step();
		title = "Final";
		step_state = StepState::Complete;
		break;
	default:
		return false;
	}
	...
	record_current_uv();
	if (step_state == StepState::Complete)
	{
...
void PolySquareDeformation::record_current_uv()
{
	std::vector<OpenMesh::Vec2d> vertex_uv(para.n_vertices());
	for (int i = 0; i < para.n_vertices(); ++i)
	{
		const int cid = v_chart[i].first;
		const int vid = v_chart[i].second;
		vertex_uv[i][0] = atlas[cid].uv_x[2 * vid + 0];
		vertex_uv[i][1] = atlas[cid].uv_x[2 * vid + 1];
	}
	vertex_uv_vec.emplace_back(std::move(vertex_uv));
}
